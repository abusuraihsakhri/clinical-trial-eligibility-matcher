"""
Command-Line Interface for TrialMatcher AI: Natural Language EHR Phenotype & Protocol Inclusion/Exclusion Agent.
"""
import argparse
import csv
import json
...
from .agents import TrialMatcherCoordinator

coordinator = TrialMatcherCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="clinical-trial-eligibility-matcher", description="TrialMatcher AI: Natural Language EHR Phenotype & Protocol Inclusion/Exclusion Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  TRIALMATCHER AI: NATURAL LANGUAGE EHR PHENOTYPE & PROTOCOL INCLUSION/EXCLUSION AGENT")
        print(f"  Domain: Clinical Research | Standard: NIH / CDISC Clinical Trial Protocols")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting TrialMatcher AI: Natural Language EHR Phenotype & Protocol Inclusion/Exclusion Agent on http://{args.host}:{args.port}")
